...
// Uses (source, source_id) as the natural key so a source rediscovering
// the same entity updates the existing row rather than duplicating.
...
async function upsertBuyer(buyer) {
  ...
  const sql = `
    ...
    ON CONFLICT (source, source_id) DO UPDATE SET
      ...
      hs_codes = (
        SELECT ARRAY(SELECT DISTINCT UNNEST(scraped_buyers.hs_codes || EXCLUDED.hs_codes))
      ),
